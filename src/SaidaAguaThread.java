...
public class SaidaAguaThread extends Thread implements Runnable {
    ...
    public SaidaAguaThread(Controlador controlador) {
    ...
    public void run() {
        while (true) {
            try {
                mudarTemperatura(enviarPacket("anf123.4"));
                Thread.sleep(50);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void mudarTemperatura(DatagramPacket packet) {
        ...
        this.controlador.agua.setText(String.format("%.2f", novaTemperatura));
    }
